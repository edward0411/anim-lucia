...
use App\Models\Gestion_ambiental_bitacora as gestion_bitacora;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Log;

class Gestiones_ambientales_bitacoraController extends Controller
{
    public function store_bitacora(Request $request)
    {

        $gestion_ambiental_bitacora = gestion_bitacora::find($request->id_bitacora);
        ...
        if ($gestion_ambiental_bitacora == null) {
            if (isset($request->id_bitacora_crear) && ($request->id_bitacora_crear == 1)) {
                $gestion_ambiental_bitacora = new gestion_bitacora();
            }else{
                $respuesta['status']="error";
                $respuesta['message']="No tiene permisos para crear registros nuevos";
                $respuesta['objecto']= $gestion_ambiental_bitacora;
                return response()->json($respuesta);
           }
        }
        $gestion_ambiental_bitacora->id_gestion_ambiental  = $request->bitacora_id_gestion_ambiental;
        $gestion_ambiental_bitacora->fecha = $request->fecha_bitacora;
        $gestion_ambiental_bitacora->Descripcion_gestion =  $request->descripcion_gestion;
        $gestion_ambiental_bitacora->vinculo =$request->vinculo;


        if ($request->id_bitacora == 0) {
          $gestion_ambiental_bitacora->created_by = Auth::user()->id;
        }else {
           $gestion_ambiental_bitacora->updated_by = Auth::user()->id;
        }
        $gestion_ambiental_bitacora->save();

        if($gestion_ambiental_bitacora->updated_by != null){
            if (!$request->file('photo')) {
                if ($gestion_ambiental_bitacora->imagen == '' || $gestion_ambiental_bitacora->imagen == null) {
                    $gestion_ambiental_bitacora->imagen = '';
                    $gestion_ambiental_bitacora->save();
                }
            } else {
                $path = public_path().'/images/GestionAmbiental_bitacora/';
                $extension = $request->file('photo')->getClientOriginalExtension();
                $filename = $gestion_ambiental_bitacora->id . '.' . $extension;
                $request->file('photo')->move($path, $filename);
                $gestion_ambiental_bitacora->imagen = $filename;
                $gestion_ambiental_bitacora->save();
            }
        }else{
            if (!$request->file('photo')) {
        ...
        $respuesta['objeto']= $gestion_ambiental_bitacora;
    ...
    public function gestion_bitacora_get_info(Request $request)
    {
        ...
        $gestion_ambiental_bitacora = gestion_bitacora::where('id_gestion_ambiental',$request->permisos_ambientales_id_bitacora)
        ->get();

        return response()->json($gestion_ambiental_bitacora);
