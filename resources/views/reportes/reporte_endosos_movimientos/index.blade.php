@extends('layouts.app',
        $vars=[ 'breadcrum' => ['Reportes','Reporte endosos movimientos'],
                'title'=>'Reporte endosos movimientos',
                'activeMenu'=>'23'
              ])
...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            <form role="form" method="POST"  action="{{route('reportes.reporte_endosos_movimientos.busqueda')}}">
                    @csrf
                    @method('POST')
            <div class="card-body">
              <div class="form-row">
            ...
            <div class="card card-primary  shadow">
                <div class="card-header">
                    <h3 class="card-title">Reporte endosos movimientos</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body" style="overflow-x: scroll;max-height: 500px;overflow-y: scroll;">
                    <table id="tabledata" class="table table-bordered table-striped" style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <th>Id Endoso</th>
                            <th>Valor del endoso</th>
                            <th>Tipo de giro</th>
                            <th>Forma de pago</th>
                            <th>Instrucciones adicionales</th>
                            <th>Ciudad tributación</th>
                            <th>Razón social endoso</th>
                            <th>Naturaleza jurídica endoso</th>
                            <th>Tipo documento tercero endoso</th>
                            <th>Identificación tercero endoso</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$busqueda->id_endoso}}</td>
                                <td>{{$busqueda->valor_endoso}}</td>
                                <td>{{$busqueda->id_param_tipo_giro_text}}</td>
                                <td>{{$busqueda->id_param_forma_pago_text}}</td>
                                <td>{{$busqueda->instrucciones_adicionales}}</td>
                                <td>{{$busqueda->ciudad_tributacion}}</td>
                                <td>{{$busqueda->razon_social_endoso}}</td>
                                <td>{{$busqueda->param_naturaleza_juridica_texto_endoso}}</td>
                                <td>{{$busqueda->tipo_documento_tercero_endoso}}</td>
                                <td>{{$busqueda->identificacion_tercero_endoso}}</td>
